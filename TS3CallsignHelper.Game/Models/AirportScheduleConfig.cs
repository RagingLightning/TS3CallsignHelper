...
public partial class AirportScheduleConfig {
  [GeneratedRegex("^(?<q0>\"?)(?<operator>\\p{Lu}{4})\\k<q0>,(?<q1>\"?)(?<airline>\\p{Lu}{4})\\k<q1>,(?<q2>\"?)(?<flightnumber>.+?)\\k<q2>,(?<q3>\"?)(?<airplanetype>.+?)\\k<q3>,(?<q4>\"?)(?<from>\\p{Lu}{4})\\k<q4>,(?<q5>\"?)(?<to>\\p{Lu}{4})\\k<q5>,(?<q6>\"?)(?<arrival>[0-9:]*?)\\k<q6>,(?<q7>\"?)(?<departure>[0-9:]*?)\\k<q7>,(?<q8>\"?)(?<approachaltitude>\\d*?)\\k<q7>,(?<q8>\"?)(?<special>.*?)\\k<q8>$")]
  private static partial Regex Parser();

  public IEnumerable<AirportScheduleEntry> Schedule => _schedule;

  private List<AirportScheduleEntry> _schedule;

  public AirportScheduleConfig(string configPath) {
    _schedule = new List<AirportScheduleEntry>();
    ...
    while(reader.ReadLine() is string line) {
      ...
      _schedule.Add(new AirportScheduleEntry(groups["operator"].Value,
